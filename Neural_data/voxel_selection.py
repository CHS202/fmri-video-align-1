...
def generate_voxel_selection(config):
    ...
    for video_id in video_order:
        ...
        for roi in config['rois']:
            ...
            if voxel_path:
                ...
                if roi == 'ALL':
                    roi_data = roi_data.T
                voxels_for_this_video.append(roi_data)
            else:
                print(f"Warning: Voxel file not found for video '{video_base_name}', ROI '{roi}'. Skipping video.")
                voxels_for_this_video = [] # Clear list to skip this video
                break
        
        # If all ROIs were found, concatenate and add them
        if voxels_for_this_video:
            concatenated_voxels = np.concatenate(voxels_for_this_video)
    ...
    output_path = os.path.join(config['output_dir'], config['output_file_name'])
...
if __name__ == '__main__':
    ...
    input_filename = f"voxel_select_remain_time_{SPLIT}.npy"
    output_filename = f"voxel_select_new_{SPLIT}.npy"
    ...
    try:
        ...
        print(f"Please make sure 'voxel_select_remain_time_{SPLIT}.npy' exists.")
