...
def pretrained_shufflenet_v1(snippet_duration: int,
                         sample_size: int,
                         n_classes,
                         pretrained_model_path):
    n_finetune_classes = 600
    model = get_model(groups=3, num_classes=600, width_mult=1.5)
    model = model.cuda()
    # model = nn.DataParallel(model, device_ids=None)
    print('Loading pretrained 3D shufflenet_v1 {}'.format(pretrained_model_path))
...
def pretrained_shufflenet_v1_1(snippet_duration: int,
                         sample_size: int,
                         n_classes,
                         pretrained_model_path):
    n_finetune_classes = 600
