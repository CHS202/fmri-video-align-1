...
def pretrained_squeezenet(snippet_duration: int,
                         sample_size: int,
                         n_classes,
                         pretrained_model_path):
    n_finetune_classes = 600
    model = SqueezeNet(version=1.1, sample_size = sample_size, sample_duration = snippet_duration, num_classes=600)
    model = model.cuda()
    print('Loading pretrained 3D squeezenet {}'.format(pretrained_model_path))
    ...
    # ---------------------------------------------------------------- #
    model.classifier[1] = nn.Conv3d(512, n_classes, kernel_size=1)
    model.classifier = model.classifier.cuda()
    return model

def pretrained_squeezenet_1(snippet_duration: int,
                         sample_size: int,
                         n_classes,
                         pretrained_model_path):
    n_finetune_classes = 600
    ...
    # ---------------------------------------------------------------- #
    # --- MODIFICATION START ---
    # Instead of just replacing one layer, we rebuild the entire classifier
    # to insert the projection head.
    
    # 1. Get references to the original, non-learnable layers.
    dropout_layer = model.classifier[0]
    final_relu = model.classifier[2]
    avg_pool = model.classifier[3]
    ...
    model.classifier = nn.Sequential(
        dropout_layer,
        # Projection Layer: Reduces channels from 512 -> projection_dim
        nn.Conv3d(in_channels, projection_dim, kernel_size=1),
        nn.ReLU(inplace=True),
        # Final Logic Output Layer: Maps from projection_dim -> n_classes
        nn.Conv3d(projection_dim, n_classes, kernel_size=1),
        final_relu,
        avg_pool
    )
    model.classifier = model.classifier.cuda()
